...
function needsOperationalFacts(intent: BotQuestionIntent): boolean {
  return intent.kind === "level_status"
    || intent.kind === "score_breakdown"
    || intent.kind === "cs_interaction_check"
    || intent.kind === "score_missing_check";
}

async function resolveFactAnswer(
  factService: BotFactService | undefined,
  openId: string,
  question: string,
): Promise<string | null> {
  const intent = classifyBotQuestionIntent(question);
  const immediate = buildFactAnswer({ intent, facts: null, question });
  if (immediate.handled) return immediate.text;

  if (!factService || !needsOperationalFacts(intent)) return null;

  const facts: OperationalFacts = await factService.getOperationalFacts({ openId, question });
  const answer = buildFactAnswer({ intent, facts, question });
  return answer.handled ? answer.text : null;
}
