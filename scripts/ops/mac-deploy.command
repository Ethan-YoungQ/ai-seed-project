...
ARCHIVE_FILE="$TMP_DIR/phase-one-deploy.tar"
...
scp "$ARCHIVE_FILE" "$TARGET:$REMOTE_DIR/phase-one-deploy.tar"
ssh "$TARGET" "cd '$REMOTE_DIR' && tar -xf phase-one-deploy.tar && rm -f phase-one-deploy.tar && bash scripts/ops/deploy-app.sh"
