...
$archiveFile = Join-Path $archiveDir "phase-one-deploy.tar"
...
scp $archiveFile "${target}:$remoteDir/phase-one-deploy.tar"
ssh $target "cd '$remoteDir' && tar -xf phase-one-deploy.tar && rm -f phase-one-deploy.tar && bash scripts/ops/deploy-app.sh"
